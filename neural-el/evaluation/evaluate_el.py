...
def computeMaxPriorContextJointEntities(
  ...
  # [[(trueWT, maxPrWT, maxContWT, maxJWT), (trueWID, maxPrWID, maxContWID, maxJWID)]]
    ...
    if numWithCorrectInCand != 0:
        ...
        accCondAll = accCond/float(numMens)
        accContAll = accCont/float(numMens)
        accJointAll = accJoint/float(numMens)
    else:
        accCond = 0.0
        accCont = 0.0
        accJoint = 0.0
    ...
    print("Linkable -- Priors Accuracy: {:.3f}  Context Accuracy: {:.3f}  Joint Accuracy: {:.3f}".format(
        (accCond), accCont, accJoint))

    print("All -- Priors Accuracy: {:.3f}  Context Accuracy: {:.3f}  Joint Accuracy: {:.3f}".format(
        (accCondAll), accContAll, accJointAll))
