...
public class TACReader extends XmlDocumentReader {
    ...
    //private static final String[] SOURCE_TYPES = {"test"};
    ...
    /*
    This is to handle a specific HTML tag in the discussion forum data which causes the reader
    to die if not explicitly handled. It can be found in ENG_DF_001488_20150107_F001000E1.df.xml.
     */
    private static final String EX_LINK = "href=“http://www.vox.com/2015/1/7/7507883/charlie-hebdo-explained-covers”";
    ...
    private static XmlTextAnnotationMaker buildXmlTextAnnotationMaker(TextAnnotationBuilder textAnnotationBuilder,
        ...
        tagsWithAtts.put(B, attributeNames);
        ...
        deletableSpanTags.add(AUTHOR);

        Set<String> tagsToIgnore = new HashSet<>();
        tagsToIgnore.add(ACK);
        tagsToIgnore.add(THREE_D);
        tagsToIgnore.add(IMG);
        tagsToIgnore.add(MARK);
        tagsToIgnore.add(I);
        tagsToIgnore.add(U);
        tagsToIgnore.add(EX_LINK);

        XmlDocumentProcessor xmlProcessor =
                new XmlDocumentProcessor(deletableSpanTags, tagsWithAtts, tagsToIgnore,
                        throwExceptionOnXmlParseFail, false);
    ...
    private static ResourceManager buildTACConfig(String corpusRoot, Language language) {
        String langSubDir;
        // language specifies which subdirectory the data will be in
        // Chinese code for TAC is inconsistent with in library language codes
