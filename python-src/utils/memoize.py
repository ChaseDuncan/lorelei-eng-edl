...
class memoize_persistent(object):
    ...
    def save(self):
        if self.cache and self.dirty:
            pickle.dump((self.cache, self.key), open(self.filename,'wb'))
            print(
                '[ATEXIT] saved persistent cache for {self.func.__name__} to file "{self.filename}"'.format(self=self))
        else:
    ...
    def load(self):
        ...
        loaded_key = None
        try:
            (cache, loaded_key) = pickle.load(open(self.filename,'r'))
        except IOError:
            pass
        finally:
